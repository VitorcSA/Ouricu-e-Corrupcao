...
int main() {
    ...
    UnloadImage(rei);
    Image reinofundo = LoadImage("assets/reino.png");
        ImageResize(&reinofundo, 1024, 720);
        Texture2D reinoFundo = LoadTextureFromImage(reinofundo);
        UnloadImage(reinofundo);
    ...
    while (!WindowShouldClose()) {
        ...
        if (tutorialAtivo) {
            ...
            DrawTexture(reinoFundo, 0, 0, WHITE);
